...
public class UpdateADAccounts implements PostProcessHandler, ConditionalEventHandler {

	private final static String className = UpdateADAccounts.class.getName();
	...
	public EventResult execute(long processId, long eventId, Orchestration orchestration) {
		...
		System.out.println("EventResult - Execute: UpdateADAccounts.");
		try {
				...
				if (!userKey.isEmpty()) {
					...
					if (resourceAccount != null) {
						...
						if (!modParentData.isEmpty()) {
							logger.info("Some information must be updated in AD for User Key: " + userKey);
		...
		logger.exiting(className, "execute UpdateADAccounts");
	...
	public BulkEventResult execute(long processId, long eventId, BulkOrchestration orchestration) {
		...
		System.out.println("BulkEventResult - Execute: UpdateADAccounts.");
		try {
			...
			if (orchestration.getOperation().equals("MODIFY")) {
				...
				Set<String> KeySet = parameters.keySet();
				/*
				 * List of parameters updated. for (String key : KeySet) { logger.fine("--" +
	...
	// * @return
	// */
	//public String toCapitalizeStripAccentString2(String str) {
    //
	...
	// */
	//public String toStripAccents2(String str) {
    //
